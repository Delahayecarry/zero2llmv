# MiniMindVLM 多模态视觉语言模型架构文档

## 🎯 模型概览

MiniMindVLM 是基于 MiniMind 大语言模型架构扩展的多模态视觉语言模型，能够同时理解和处理文本与图像信息。
...
flowchart TD
    ...
    subgraph Transformer["🧠 统一Transformer处理"]
        ...
        D3["多层Transformer<br/>⚡ N × MiniMindBlock<br/>🎯 注意力 + FFN/MoE"]
...
flowchart TD
    subgraph TransformerFlow["🧠 Transformer多模态处理流程"]
        ...
        %% 多层处理
        T3["MiniMindBlock 0<br/>🎯 Attention + FFN/MoE<br/>⚡ 自注意力机制"]
        T4["MiniMindBlock 1<br/>🎯 Attention + FFN/MoE<br/>⚡ 跨模态信息交互"] 
        T5["MiniMindBlock ...<br/>🎯 Attention + FFN/MoE<br/>⚡ 深层语义理解"]
        T6["MiniMindBlock N-1<br/>🎯 Attention + FFN/MoE<br/>⚡ 最终特征提取"]
...
classDiagram
    ...
    CausalLM <|-- MiniMindVLM
    torch_nn_Module <|-- VisionEncoder
    
    %% 组合关系
    MiniMindVLM *-- VLLMconfig
    MiniMindVLM *-- VisionEncoder
    MiniMindVLM *-- CLIPModel
    MiniMindVLM *-- CLIPProcessor
    
    %% 依赖关系
    VisionEncoder ..> torch_nn_Module : uses
    MiniMindVLM ..> torch : uses
```
...
sequenceDiagram
    ...
    participant Model as 🧠 MiniMindVLM
...
graph TD
    subgraph Comparison["📊 模型架构对比"]
        ...
        subgraph MiniMindVLM["🚀 MiniMindVLM方案"]
