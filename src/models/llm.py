...
sys.path.insert(0, str(Path("src").resolve())) # 从src开始找
    ...
    def forward(
        ...
        if False and self.flash and seq_len != 1:
            ...
            if attention_mask is not None:
                ...
                total_seq_len = xk.shape[2]  # 获取实际的key序列长度
...
# # =================测试==================
# import torch
# from src.zero2llmv.models.llm import CausalLM
# from src.zero2llmv.models.llmconfig import llmconfig
#
#
# def main():
#     """
#     一个用于直接测试和调试CausalLM模型的main函数。
#
#     该函数将执行以下操作：
#     1. 创建一个LLM的配置对象。
#     2. 使用该配置实例化CausalLM模型。
#     3. 创建一个模拟的输入数据（input_ids, attention_mask）。
#     4. 执行模型的前向传播，并打印输出的形状。
#     5. 展示如何使用PyCharm的调试功能来检查中间张量。
#     """
#
#     print("--- 开始模型测试 ---")
#
#     # 1. 创建模型配置对象
#     # 我们可以根据需要修改这些参数来测试不同的模型规模
#     # 这是一个小型的演示配置
#     config = llmconfig(
#         vocab_size=1000,
#         hidden_size=256,
...
#         num_key_value_heads=2,  # 使用分组查询注意力
#         intermediate_size=512,
#         max_position_embeddings=512,
#         dropout=0.1,
#         use_moe=False,
#     )
#     print(f"模型配置: {config.num_hidden_layers} 层, {config.hidden_size} 维度")
#
#     # 2. 实例化CausalLM模型
#     try:
...
#
#     # 3. 创建模拟输入数据
#     # 模拟一个batch_size=2，序列长度seq_len=10的输入
#     batch_size = 2
#     seq_len = 10
#
#     # 创建一个随机的输入token ID张量，范围在0到vocab_size之间
#     input_ids = torch.randint(0, config.vocab_size, (batch_size, seq_len))
#
#     # 创建一个注意力掩码，用于模拟padding。
#     # 这里的掩码表示所有token都是可见的（没有padding）。
#     # 如果想测试padding，可以将部分值设为0。
...
#
#     print(f"\n输入张量形状: input_ids={input_ids.shape}, attention_mask={attention_mask.shape}")
#
#     # 4. 执行模型前向传播
#     # 在这里，你可以设置一个断点，然后用PyCharm的调试功能来检查中间张量。
#     # 例如，你可以在 `model.forward` 函数内部设置断点，
#     # 逐步执行并检查 `h` 和 `logits` 等张量的形状。
#     try:
#         # ** 在这一行设置断点 **
#         output = model(
#             input_ids=input_ids,
#             attention_mask=attention_mask,
#             use_cache=True,  # 模拟生成模式，返回KV缓存
...
#
#         # 检查KV缓存的形状
#         # past_key_values 是一个列表，每个元素是(key_cache, value_cache)元组
#         print(f"输出KV缓存的层数: {len(output.past_key_values)}")
#         first_layer_kv = output.past_key_values[0]
#         if first_layer_kv:
#             print(f"第一层的key缓存形状: {first_layer_kv[0].shape}")
#             print(f"第一层的value缓存形状: {first_layer_kv[1].shape}")
#
#     except Exception as e:
#         print(f"\n模型前向传播失败: {e}")
#
